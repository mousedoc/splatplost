...
def systemctl_output(*arguments: str) -> str:
    """Collect Linux diagnostics without breaking the report dialog on Windows."""
    if shutil.which("systemctl") is None:
        return "Not available on this platform."
    try:
        return subprocess.check_output(
            ["systemctl", *arguments], stderr=subprocess.STDOUT, text=True, timeout=10
        )
    except (OSError, subprocess.SubprocessError) as error:
        return f"Unable to collect systemctl diagnostics: {error}"
...
```
        """.format(__version__,
                   sys.version,
                   platform.platform(),
                   systemctl_output("--version"),
                   traceback_text,
                   systemctl_output("status", "bluetooth.service"),
                   )
